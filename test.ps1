...
while ($continue -match 'yes'){
	...
	if ($studentOrStaff -eq 'staff') {
		...
		New-ADUser -Server blrv3 -Credential $cred -Name $fullName -GivenName $firstName -Surname $lastName -AccountPassword $securepassword -DisplayName $fullName -Enabled $true -Description $description -Organization $studentou
		
		$UserDN  = (Get-ADUser -Server blrv3 -Credential $cred -Identity $fullName)
		Move-ADObject -Server blrv3 -Credential $cred -Identity $UserDN  -TargetPath $studentou
	} else {
